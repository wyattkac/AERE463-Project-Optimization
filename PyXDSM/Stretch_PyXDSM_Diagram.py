...
x.connect("vspaero", "opt", [r"\text{Cl}",
                             r"\text{Cd}"])
x.add_system("fea", FUNC, r"\text{OpenVSP (FEA)}")
x.connect("openvsp", "fea", [r"\text{Structure}"])
x.connect("vspaero", "fea", [r"\text{Aero Loads}"])
x.connect("fea","opt",[r"\text{Stress}"])
